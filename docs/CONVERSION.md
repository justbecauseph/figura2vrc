...
| 11 | A group of parts renders white/untextured in Unity (while looking fine in Blockbench) | Blockbench's glTF exporter can **embed the wrong image** for a material — the face/material assignments are correct but the packed PNG is a different texture entirely (the converter warns when an extracted image matches no source PNG). Everything sharing that material breaks together, which can look like several unrelated broken parts | Assign the correct source PNG to that material in Unity (the geometry and UVs are fine). Mesh, weights, and hierarchy survive the exporter correctly — verified — so white parts are always a texture problem, not a rig problem |
| 12 | Parts look hollow / see-through in Unity but solid in Blockbench | Authors UV hidden faces (box tops/backs) to transparent texture corners; Blockbench previews double-sided so the interior masks it, Unity culls backfaces and opens a window into the hollow box | Use the included `MinecraftUnlit` shader — it renders double-sided by default (Cull Off) — or paint the transparent UV regions in the skin |
